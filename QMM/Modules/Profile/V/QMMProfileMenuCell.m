...
@interface QMMProfileMenuCell()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation QMMProfileMenuCell
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    NSArray *arr = (NSArray *)self.cellModel.value;
    return arr.count;
}

- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    NSArray *arr = self.cellModel.value;
    QMMProfileCellModel *model = arr[indexPath.item];
...
- (void)setupSubviews {
    ...
    _infoLabel = [UILabel labelWithText:self.cellModel.desc
                              ...
                              tapAction:^(UILabel *label, UIGestureRecognizer *tap) {
                                  NSArray *arr = self.cellModel.value;
                                  QMMProfileCellModel *model = arr[0];
                                  if (self.menuItemClick) {
