...
#import "QMMUserInfoVC.h"
#import "QMMUserInfoCellModel.h"
#import "QMMUserInfoHeaderCell.h"
#import "QMMUserInfoPhotosCell.h"
#import "QMMUserInfoCell.h"
#import "QMMUserInfoListCell.h"
#import "QMMUserInfoDescCell.h"
#import "QMMUserInfoTagCell.h"
...
@interface QMMUserInfoVC ()

@property (nonatomic, strong) QMMUserInfoVM *viewModel;
...
- (void)initialize {
    self.viewModel = [QMMUserInfoVM new];
    self.viewModel.uid = self.uid;
    self.viewModel.type = self.type;
    
    self.imgPicker = [ImagePickerHelper new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QMMUserInfoCellModel *cellModel = sections[indexPath.row];
    @weakify(self);
    
    switch (cellModel.cellType) {
        case UserInfoCellTypeHeader: {
            QMMUserInfoHeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoHeaderCellReuseID];
            cell.cellModel = cellModel;
            return cell;
            break;
        }
        case UserInfoCellTypeInfo: {
            QMMUserInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoCellReuseID];
            cell.cellModel = cellModel;
            return cell;
            break;
        }
        case UserInfoCellTypePhotos: {
            QMMUserInfoPhotosCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoPhotosCellReuseID];
        ...
        case UserInfoCellTypeList: {
            QMMUserInfoListCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoListCellReuseID];
            cell.cellModel = cellModel;
            return cell;
            break;
        }
        case UserInfoCellTypeDesc: {
            QMMUserInfoDescCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoDescCellReuseID];
            cell.cellModel = cellModel;
            return cell;
        }
        case UserInfoCellTypeTags: {
            QMMUserInfoTagCell *cell = [tableView dequeueReusableCellWithIdentifier:kUserInfoTagCellReuseID];
            cell.cellModel = cellModel;
            return cell;
        }
        default:
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cellModel.cellType == UserInfoCellTypeDesc) {
        return [tableView fd_heightForCellWithIdentifier:kUserInfoDescCellReuseID configuration:^(QMMUserInfoDescCell *cell) {
            cell.cellModel = cellModel;
        }];
    }
    else if (cellModel.cellType == UserInfoCellTypeTags) {
        return [tableView fd_heightForCellWithIdentifier:kUserInfoTagCellReuseID configuration:^(QMMUserInfoTagCell *cell) {
            cell.cellModel = cellModel;
        }];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellModel.title isEqualToString:@"所在地区"]) {
        YSPickerView *picker = [YSPickerView pickerViewWithType:YSPickerViewTypeTriple];
        [picker showPickerViewWithDataArray:[YSPickerViewData shareData].places sureHandle:^(NSArray *arr) {
            YSPickerViewModel *m0 = arr[0];
            YSPickerViewModel *m1 = arr[1];
            YSPickerViewModel *m2 = arr[2];
...
- (void)registerCell {
    [self.tableView registerClass:[QMMUserInfoHeaderCell class] forCellReuseIdentifier:kUserInfoHeaderCellReuseID];
    [self.tableView registerClass:[QMMUserInfoCell class] forCellReuseIdentifier:kUserInfoCellReuseID];
    [self.tableView registerClass:[QMMUserInfoPhotosCell class] forCellReuseIdentifier:kUserInfoPhotosCellReuseID];
    [self.tableView registerClass:[QMMUserInfoListCell class] forCellReuseIdentifier:kUserInfoListCellReuseID];
    [self.tableView registerClass:[QMMUserInfoDescCell class] forCellReuseIdentifier:kUserInfoDescCellReuseID];
    [self.tableView registerClass:[QMMUserInfoTagCell class] forCellReuseIdentifier:kUserInfoTagCellReuseID];
...
- (void)fekeRequest {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [YSProgressHUD showTips:@"谢谢您的举报，我们会及时核查"];
    });
}
...
- (void)setupSubviews {
    ...
    CGFloat wh = 25;
    self.backBtn = ({
        UIButton *btn = [UIButton buttonWithNormalImgName:@"ic_nav_back"
